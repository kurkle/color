...
describe('Color class', () => {
  describe('constructor', () => {
    ...
    it('should parse hex string correctly', () => {
      ...
      expect(color.rgb).toEqual({ a: 1, b: 0, g: 0, r: 255 })
    })

    it('should parse rgb string correctly', () => {
      const color = new Color('rgb(0, 255, 0)')
      expect(color.rgb).toEqual({ a: 1, b: 0, g: 255, r: 0 })
    })

    it('should parse hsl string correctly', () => {
      const color = new Color('hsl(240, 100%, 50%)')
      expect(color.rgb).toEqual({ a: 1, b: 255, g: 0, r: 0 })
    })

    it('should handle RGB object input', () => {
      const color = new Color({ a: 0.5, b: 0, g: 0, r: 255 })
      expect(color.rgb).toEqual({ a: 0.5, b: 0, g: 0, r: 255 })
    })
    ...
    it('should handle RGB array input', () => {
      ...
      expect(colorWithAlpha.rgb).toEqual({ a: 0.5, b: 0, g: 0, r: 255 })
    })
  ...
  describe('rgb property', () => {
    ...
    it('should set the rgb value correctly', () => {
      const color = new Color('#ff0000')
      color.rgb = { a: 0.5, b: 0, g: 255, r: 0 }
      expect(color.rgb).toEqual({ a: 0.5, b: 0, g: 255, r: 0 })
    })
  })
  ...
  describe('color manipulation methods', () => {
    describe('mix', () => {
      ...
      it('should return the same color if no color to mix with', () => {
        ...
        // biome-ignore lint/suspicious/noTsIgnore: strictNullChecks is off, so this never actually errors; ts-expect-error would fail as unused
