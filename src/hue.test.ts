...
describe('hue.ts', () => {
  describe('rgb2hsl', () => {
    it('should convert RGB to HSL correctly', () => {
      // Red
      expect(rgb2hsl({ a: 255, b: 0, g: 0, r: 255 })).toEqual([0, 1, 0.5])

      // Green
      expect(rgb2hsl({ a: 255, b: 0, g: 255, r: 0 })).toEqual([120, 1, 0.5])

      // Blue
      expect(rgb2hsl({ a: 255, b: 255, g: 0, r: 0 })).toEqual([240, 1, 0.5])

      // White
      expect(rgb2hsl({ a: 255, b: 255, g: 255, r: 255 })).toEqual([0, 0, 1])

      // Black
      expect(rgb2hsl({ a: 255, b: 0, g: 0, r: 0 })).toEqual([0, 0, 0])

      // Gray
      expect(rgb2hsl({ a: 255, b: 128, g: 128, r: 128 })).toEqual([0, 0, 0.5019607843137255])

      // Purple
      expect(rgb2hsl({ a: 255, b: 128, g: 0, r: 128 })).toEqual([300, 1, 0.25098039215686274])

      // Yellow
      expect(rgb2hsl({ a: 255, b: 0, g: 255, r: 255 })).toEqual([60, 1, 0.5])

      // Cyan
      expect(rgb2hsl({ a: 255, b: 255, g: 255, r: 0 })).toEqual([180, 1, 0.5])

      // Magenta
      expect(rgb2hsl({ a: 255, b: 255, g: 0, r: 255 })).toEqual([300, 1, 0.5])
    })

    it('should handle alpha channel correctly', () => {
      // Alpha should not affect HSL values
      expect(rgb2hsl({ a: 128, b: 0, g: 0, r: 255 })).toEqual([0, 1, 0.5])
    })
  })
  ...
  describe('rotate', () => {
    it('should rotate colors correctly', () => {
      // Rotate red to green
      const red: RGBA = { a: 255, b: 0, g: 0, r: 255 }
      rotate(red, 120)
      expect(red).toEqual({ a: 255, b: 0, g: 255, r: 0 })

      // Rotate green to blue
      const green: RGBA = { a: 255, b: 0, g: 255, r: 0 }
      rotate(green, 120)
      expect(green).toEqual({ a: 255, b: 255, g: 0, r: 0 })

      // Rotate blue to red
      const blue: RGBA = { a: 255, b: 255, g: 0, r: 0 }
      rotate(blue, 120)
      expect(blue).toEqual({ a: 255, b: 0, g: 0, r: 255 })
    })
    ...
    it('should handle negative rotation', () => {
      ...
      const red: RGBA = { a: 255, b: 0, g: 0, r: 255 }
      rotate(red, -120)
      expect(red).toEqual({ a: 255, b: 255, g: 0, r: 0 })
    })
    ...
    it('should not change alpha', () => {
      ...
      const red: RGBA = { a: 128, b: 0, g: 0, r: 255 }
      rotate(red, 120)
  ...
  describe('hslString', () => {
    it('should convert RGBA to HSL string correctly', () => {
      // Red
      expect(hslString({ a: 255, b: 0, g: 0, r: 255 })).toBe('hsl(0, 100%, 50%)')

      // Green
      expect(hslString({ a: 255, b: 0, g: 255, r: 0 })).toBe('hsl(120, 100%, 50%)')

      // Blue
      expect(hslString({ a: 255, b: 255, g: 0, r: 0 })).toBe('hsl(240, 100%, 50%)')

      // White
      expect(hslString({ a: 255, b: 255, g: 255, r: 255 })).toBe('hsl(0, 0%, 100%)')

      // Black
      expect(hslString({ a: 255, b: 0, g: 0, r: 0 })).toBe('hsl(0, 0%, 0%)')

      // Gray
      expect(hslString({ a: 255, b: 128, g: 128, r: 128 })).toBe('hsl(0, 0%, 50%)')
    })

    it('should convert RGBA to HSLA string when alpha < 255', () => {
      // Red with 50% opacity
      expect(hslString({ a: 128, b: 0, g: 0, r: 255 })).toBe('hsla(0, 100%, 50%, 0.5)')

      // Green with 75% opacity
      expect(hslString({ a: 191, b: 0, g: 255, r: 0 })).toBe('hsla(120, 100%, 50%, 0.75)')

      // Blue with 25% opacity
      expect(hslString({ a: 64, b: 255, g: 0, r: 0 })).toBe('hsla(240, 100%, 50%, 0.25)')

      // Fully transparent
      expect(hslString({ a: 0, b: 0, g: 0, r: 255 })).toBe('hsla(0, 100%, 50%, 0)')
    })
