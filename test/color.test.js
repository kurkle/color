...
describe('Color Parsing', () => {
  describe('Hex Color Parsing', () => {
    it('should parse 3-digit hex colors correctly', () => {
      expect(new Color('#fef').rgb).toEqual({ a: 1, b: 255, g: 238, r: 255 })
      expect(new Color('#123').rgb).toEqual({ a: 1, b: 51, g: 34, r: 17 })
    })

    it('should parse 6-digit hex colors correctly', () => {
      expect(new Color('#fffFEF').rgb).toEqual({ a: 1, b: 239, g: 255, r: 255 })
      expect(new Color('#112233').rgb).toEqual({ a: 1, b: 51, g: 34, r: 17 })
    })

    it('should parse 4-digit hex colors with alpha correctly', () => {
      expect(new Color('#feff').rgb).toEqual({ a: 1, b: 255, g: 238, r: 255 })
      expect(new Color('#fef0').rgb).toEqual({ a: 0, b: 255, g: 238, r: 255 })
    ...
    it('should parse 8-digit hex colors with alpha correctly', () => {
      expect(new Color('#c814e933').rgb).toEqual({ a: 0.2, b: 233, g: 20, r: 200 })
      expect(new Color('#c814e900').rgb).toEqual({ a: 0, b: 233, g: 20, r: 200 })
      expect(new Color('#c814e9ff').rgb).toEqual({ a: 1, b: 233, g: 20, r: 200 })
  ...
  describe('RGB Color Parsing', () => {
    it('should parse rgb() format correctly', () => {
      expect(new Color('rgb(244, 233, 100)').rgb).toEqual({ a: 1, b: 100, g: 233, r: 244 })
      expect(new Color('rgb(255, 0, 0)').rgb).toEqual({ a: 1, b: 0, g: 0, r: 255 })
      expect(new Color('rgb(0 255 0)').rgb).toEqual({ a: 1, b: 0, g: 255, r: 0 })
    })

    it('should parse rgb() with percentages correctly', () => {
      expect(new Color('rgb(100%, 30%, 90%)').rgb).toEqual({ a: 1, b: 229, g: 77, r: 255 })
    })

    it('should parse rgba() format correctly', () => {
      expect(new Color('rgba(200, 20, 233, 0.2)').rgb).toEqual({ a: 0.2, b: 233, g: 20, r: 200 })
      expect(new Color('rgba(200, 20, 233, 0)').rgb).toEqual({ a: 0, b: 233, g: 20, r: 200 })
      expect(new Color('rgba(200, 20, 233, 1e-15)').rgb).toEqual({ a: 0, b: 233, g: 20, r: 200 })
      expect(new Color('rgba(255, 0, 0, .5)').rgb).toEqual({ a: 0.5, b: 0, g: 0, r: 255 })
    })

    it('should parse rgba() with percentages correctly', () => {
      expect(new Color('rgba(100%, 30%, 90%, 0.2)').rgb).toEqual({ a: 0.2, b: 229, g: 77, r: 255 })
      expect(new Color('rgb(0 0 100% / 80%)').rgb).toEqual({ a: 0.8, b: 255, g: 0, r: 0 })
    })

    it('should handle comment blocks in rgb/rgba values', () => {
      expect(new Color('rgba(10, 20, 30, /* comment, block */ 0.5)').rgbString()).toBe(
        'rgba(10, 20, 30, 0.5)'
      )
      expect(new Color('rgba(10 /* r */, 20 /* g */, 30 /* b */, /* a */ 0.5)').rgbString()).toBe(
        'rgba(10, 20, 30, 0.5)'
      )
    })
  ...
  describe('HSL Color Parsing', () => {
    it('should parse hsl() format correctly', () => {
      expect(new Color('hsl(240, 100%, 50.5%)').rgb).toEqual({ a: 1, b: 255, g: 3, r: 3 })
      expect(new Color('hsl(240deg, 100%, 50.5%)').rgb).toEqual({ a: 1, b: 255, g: 3, r: 3 })
      expect(new Color('hsl(0deg 100% 50%)').rgb).toEqual({ a: 1, b: 0, g: 0, r: 255 })
      expect(new Color('hsl(100, 50%, 50%)').rgb).toEqual({ a: 1, b: 64, g: 191, r: 106 })
    })

    it('should parse hsla() format correctly', () => {
      expect(new Color('hsla(12, 10%, 50%, .3)').rgb).toEqual({ a: 0.3, b: 115, g: 120, r: 140 })
      expect(new Color('hsla(12, 10%, 50%, 30%)').rgb).toEqual({ a: 0.3, b: 115, g: 120, r: 140 })
      expect(
        new Color('hsla(240.0384227364303, 4.12843378950259%, 48.71943371204973%, 0.3)').rgb
      ).toEqual({ a: 0.3, b: 129, g: 119, r: 119 })
    })
  ...
  describe('Other Color Format Parsing', () => {
    it('should parse hwb() format correctly', () => {
      expect(new Color('hwb(240, 100%, 50.5%)').rgb).toEqual({ a: 1, b: 169, g: 169, r: 169 })
      expect(new Color('hwb(244, 100%, 100%, 0.6)').rgb).toEqual({ a: 0.6, b: 128, g: 128, r: 128 })
    })

    it('should parse hsv() format correctly', () => {
      expect(new Color('hsv(240, 100%, 50.5%)').rgb).toEqual({ a: 1, b: 129, g: 0, r: 0 })
      expect(new Color('hsv(244, 100%, 100%, 0.6)').rgb).toEqual({ a: 0.6, b: 255, g: 0, r: 17 })
    })

    it('should parse named colors correctly', () => {
      expect(new Color('blue').rgb).toEqual({ a: 1, b: 255, g: 0, r: 0 })
      expect(new Color('BluE').rgb).toEqual({ a: 1, b: 255, g: 0, r: 0 })
      expect(new Color('moccasin').rgb).toEqual({ a: 1, b: 181, g: 228, r: 255 })
      expect(new Color('transparent').rgb).toEqual({ a: 0, b: 0, g: 0, r: 0 })
    })
  ...
  describe('Object and Array Parsing', () => {
    it('should parse RGB object correctly', () => {
      ...
      expect(new Color({ a: 0.3, b: 253, g: 254, r: 255 }).rgb).toEqual({
        a: 0.3,
        b: 253,
        g: 254,
        r: 255,
    ...
    it('should parse RGB array correctly', () => {
      expect(new Color([255, 254, 253]).rgb).toEqual({ a: 1, b: 253, g: 254, r: 255 })
      expect(new Color([255, 254, 253, 0.5]).rgb).toEqual({ a: 0.5, b: 253, g: 254, r: 255 })
    })
  })
})
...
describe('Color Manipulation', () => {
  describe('Alpha Manipulation', () => {
    it('should adjust alpha correctly', () => {
      expect(new Color('#7743CE').alpha(0.5).lighten(0.5).hslString()).toBe(
        'hsla(262, 58%, 80%, 0.5)'
      )
      expect(new Color('rgba(10, 10, 10, 0.8)').clearer(0.5).rgbString()).toBe(
        'rgba(10, 10, 10, 0.4)'
      )
  ...
  describe('Color Interpolation', () => {
    it('should interpolate between colors correctly', () => {
      ...
      expect(new Color('red').interpolate(new Color('green'), 0.25).rgbString()).toBe(
        'rgb(225, 65, 0)'
      )
      expect(new Color('red').interpolate(new Color('green'), 0.5).rgbString()).toBe(
        'rgb(188, 92, 0)'
      )
      expect(new Color('red').interpolate(new Color('green'), 0.75).rgbString()).toBe(
        'rgb(137, 112, 0)'
      )
...
describe('Edge Cases and Invalid Inputs', () => {
  ...
  it('should handle extreme RGB values', () => {
    expect(new Color('rgb(255, 255, 255)').rgb).toEqual({ a: 1, b: 255, g: 255, r: 255 })
    expect(new Color('rgb(0, 0, 0)').rgb).toEqual({ a: 1, b: 0, g: 0, r: 0 })
  })

  it('should handle extreme HSL values', () => {
    expect(new Color('hsl(0, 100%, 100%)').rgb).toEqual({ a: 1, b: 255, g: 255, r: 255 })
    expect(new Color('hsl(0, 0%, 0%)').rgb).toEqual({ a: 1, b: 0, g: 0, r: 0 })
    expect(new Color('hsl(360, 100%, 50%)').rgb).toEqual({ a: 1, b: 0, g: 0, r: 255 })
  })
  ...
  it('should handle alpha clamping correctly', () => {
    expect(new Color('rgba(255, 0, 0, 1.5)').rgb).toEqual({ a: 1, b: 0, g: 0, r: 255 })
    expect(new Color('rgba(255, 0, 0, -0.5)').rgb).toEqual({ a: 0, b: 0, g: 0, r: 255 })
  })
})
